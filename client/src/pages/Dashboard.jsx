...
import { Wallet, TrendingUp, CreditCard, Activity, Target, AlertCircle } from 'lucide-react';
        ...
        const fetchData = async () => {
            try {
                ...
                const [expRes, budRes] = await Promise.all([
                    ...
                    axios.get(`${API_URL}/api/budget`, { headers: { 'x-auth-token': token } })
                ]);
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="card h-full">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <Link to="/budget" className="text-sm text-accent hover:text-accent-hover font-medium flex items-center gap-1"><Target size={14} /> Manage</Link>
                    </div>
                            ...
                            return (
                                <div key={budget._id} className="space-y-1.5">
                                    <div className="flex justify-between text-sm">
                                        ...
                                        <span className={isOver ? 'text-red-400' : 'text-text-secondary'}>{currencySymbol}{budget.spent.toFixed(0)} / {currencySymbol}{budget.amount.toFixed(0)}</span>
                                    </div>
                                    <div className="w-full h-2 bg-white/5 rounded-full overflow-hidden">
                                        <div className={`h-full transition-all duration-500 ${isOver ? 'bg-red-500' : 'bg-accent'}`} style={{ width: `${percent}%` }}></div>
                                    </div>
                                </div>
                            );
                        })}
                        {budgets.length === 0 && <p className="text-text-secondary text-center py-6 italic">No budgets set.</p>}
                    </div>
                </div>
